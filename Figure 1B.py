...
h = state[:, 1]
hs = state[:, 2]
I_Na_pre = sodium_current(v, m_inf(v), parameters, h=h, hs=hs)
...
h = state[:, 1]
hs = state[:, 2]
I_Na_during = sodium_current(v, m_inf(v), parameters, h=h, hs=hs)
# (parameters[1] * (m_inf(v_list) ** 3) * h * hs_list * (v_list - parameters[4]))  # without h & hs_list matches https://www.physiology.org/doi/full/10.1152/jn.00513.2009?url_ver=Z39.88-2003&rfr_id=ori:rid:crossref.org&rfr_dat=cr_pub%3dpubmed
...
h = state[:, 1]
hs = state[:, 2]
I_Na_post = sodium_current(v, m_inf(v), parameters, h=h, hs=hs)
# (parameters[1] * (m_inf(v_list) ** 3) * h * hs_list * (v_list - parameters[4]))  # without h & hs_list matches https://www.physiology.org/doi/full/10.1152/jn.00513.2009?url_ver=Z39.88-2003&rfr_id=ori:rid:crossref.org&rfr_dat=cr_pub%3dpubmed
